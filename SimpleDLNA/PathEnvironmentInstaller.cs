  ...
  public class PathEnvironmentInstaller : Installer
  {
    ...
    public override void Install(IDictionary stateSaver)
    {
      ...
      using (var registry = Registry.CurrentUser.OpenSubKey(REG_ENV, true))
      {
        ...
        if (!string.IsNullOrWhiteSpace(path)) newpath = $"{path};{newpath}";
    ...
    public override void Rollback(IDictionary savedState)
    {
      ...
      if (!savedState.Contains(ENV_PATH)) return;
      using (var registry = Registry.CurrentUser.OpenSubKey(REG_ENV, true))
      {
        registry?.SetValue(
    ...
    public override void Uninstall(IDictionary savedState)
    {
      ...
      using (var registry = Registry.CurrentUser.OpenSubKey(REG_ENV, true))
      {
        ...
        if (StringComparer.CurrentCultureIgnoreCase.Equals(path, cleaned)) return;
