  ...
  internal sealed partial class FormServer : Form
  {
    ...
    public ServerDescription Description
    {
      get
      {
        ...
        if (checkVideo.Checked) types |= DlnaMediaTypes.Video;
        if (checkAudio.Checked) types |= DlnaMediaTypes.Audio;
        if (checkImages.Checked) types |= DlnaMediaTypes.Image;
    ...
    private void AddOrderItems()
    {
      ...
      foreach (var v in items)
      {
        ...
        if (v.Name == "title") comboOrder.SelectedIndex = i;
      }
    }
    ...
    private void AddViewItems()
    {
      var items = from v in ViewRepository.ListItems()
        orderby v.Key
        select v.Value;
      foreach (var v in items) comboNewView.Items.Add(v);
    }
    ...
    private void buttonAddDirectory_Click(object sender, EventArgs e)
    {
      if (folderDialog.ShowDialog() == DialogResult.OK)
      {
        ...
        var found = from ListViewItem i in listDirectories.Items
          ...
          select i;
        if (!found.Any()) listDirectories.Items.Add(path);
      }
    ...
    {
      foreach (var i in listDirectories.SelectedItems) listDirectories.Items.Remove((ListViewItem) i);
      SizeDirectoryColumn();
    }
    ...
    {
      foreach (var i in listViews.SelectedItems) listViews.Items.Remove((ListViewItem) i);
      SizeColumns(listViews);
    }
    ...
    private static void SizeColumns(ListView lv)
    {
      ...
      if (lv.Items.Count == 0) mode = ColumnHeaderAutoResizeStyle.HeaderSize;
      for (var c = 0; c < lv.Columns.Count; ++c) lv.Columns[c].AutoResize(mode);
    }

    private void SizeDirectoryColumn()
    {
      var mode = ColumnHeaderAutoResizeStyle.ColumnContent;
      if (listDirectories.Items.Count == 0) mode = ColumnHeaderAutoResizeStyle.HeaderSize;
