  ...
  internal class ProgramIcon : Logging, IDisposable
  {
    ...
    public ProgramIcon()
    {
      try
      {
        ...
        if (window == IntPtr.Zero) throw new Exception("Cannot get console window");
        ...
        if (iconLg == IntPtr.Zero) throw new Exception("Failed to load large icon");
        var desired = SafeNativeMethods.GetSystemMetrics(49);
        var iconSm = SafeNativeMethods.LoadImage(
          inst, "#32512", 1, desired, desired, 0);
        if (iconLg == IntPtr.Zero) throw new Exception("Failed to load small icon");
