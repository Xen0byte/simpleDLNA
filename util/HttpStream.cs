  ...
  public class HttpStream : Stream, IDisposable
  {
    ...
    public override long Length
    {
      get
      {
        ...
        if (length.Value < 0) throw new IOException("Stream does not feature a length");
        return length.Value;
      }
    }
    ...
    {
      if (offset < 0) throw new IOException("Position cannot be negative");
      ...
      if (referrer != null) request.Referer = referrer.ToString();
      ...
      if (method != HttpMethod.HEAD)
      {
        ...
        if (responseStream == null) throw new IOException("Didn't get a response stream");
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      try
      {
        if (responseStream == null) OpenAt(position, HttpMethod.GET);
    ...
    public override long Seek(long offset, SeekOrigin origin)
    {
      ...
      if (np < 0 || np >= Length) throw new IOException("Invalid seek; out of stream bounds");
      var off = position - np;
      if (off == 0)
      {
