...
namespace NMaier.SimpleDlna.Utilities
{
    ...
    where TInterface : class, IRepositoryItem
  {
    ...
    public static TInterface Lookup(string name)
    {
      ...
      TInterface result;
      if (!items.TryGetValue(name, out result)) throw new RepositoryLookupException(name);
      if (argsplit.Length == 1 || !(result is IConfigurable)) return result;
      ...
      if (ctor == null) throw new RepositoryLookupException(name);
      try
      {
        var item = ctor.Invoke(new object[] { }) as TInterface;
        if (item == null) throw new RepositoryLookupException(name);
