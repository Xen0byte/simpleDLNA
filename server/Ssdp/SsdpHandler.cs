...
namespace NMaier.SimpleDlna.Server.Ssdp
{
  internal sealed class SsdpHandler : Logging, IDisposable
  {
    ...
    public void Dispose()
    {
      ...
      running = false;
      while (messageQueue.Count != 0) datagramPosted.WaitOne();
    ...
    private void ProcessQueue(object sender, ElapsedEventArgs e)
    {
      ...
      {
        Datagram msg;
        if (!messageQueue.TryPeek(out msg)) continue;
        if (msg != null && (running || msg.Sticky))
        {
          msg.Send();
          if (msg.SendCount > DATAGRAMS_PER_MESSAGE) messageQueue.TryDequeue(out msg);
          break;
        }
    ...
    private void ReceiveCallback(IAsyncResult result)
    {
      try
      {
        ...
        if (received == null) throw new IOException("Didn't receive anything");
        if (received.Length == 0) throw new IOException("Didn't receive any bytes");
#if DUMP_ALL_SSDP
          ...
          new MemoryStream(received), Encoding.ASCII))
        {
          ...
          if (proto == null) throw new IOException("Couldn't read protocol line");
          proto = proto.Trim();
          if (string.IsNullOrEmpty(proto)) throw new IOException("Invalid protocol line");
          ...
          Debug(headers);
#endif
          if (method == "M-SEARCH") RespondToSearch(endpoint, headers["st"]);
        }
      }
      ...
      string message, bool sticky)
    {
      ...
      if (messageQueue.Count == 0) dgram.Send();
    ...
    {
      Debug("NotifyAll");
      foreach (var d in Devices) NotifyDevice(d, "alive", false);
    }
      ...
      IPAddress address)
    {
      ...
      lock (devices)
      {
        if (!devices.TryGetValue(uuid, out list)) devices.Add(uuid, list = new List<UpnpDevice>());
      }
    ...
    {
      if (req == "ssdp:all") req = null;

      DebugFormat("RespondToSearch {0} {1}", endpoint, req);
      foreach (var d in Devices)
      {
        if (!string.IsNullOrEmpty(req) && req != d.Type) continue;
    ...
    internal void UnregisterNotification(Guid uuid)
    {
      ...
      lock (devices)
      {
        if (!devices.TryGetValue(uuid, out dl)) return;
        devices.Remove(uuid);
      }

      foreach (var d in dl) NotifyDevice(d, "byebye", true);
