...
namespace NMaier.SimpleDlna.Server
{
    ...
    : Logging, IMediaServer, IPrefixHandler
  {
    ...
    public MediaMount(IMediaServer aServer)
    {
      ...
      if (vms != null) vms.Changed += ChangedServer;
    }
    ...
    public IResponse HandleRequest(IRequest request)
    {
      ...
      if (path == "control") return ProcessSoapRequest(request);
      ...
      if (string.IsNullOrEmpty(path) || path == "index.html") return new Redirect(request, Prefix + "index/0");
      ...
      if (request.Method == "UNSUBSCRIBE") return new StringResponse(HttpCode.Ok, string.Empty);
    ...
    private string GenerateDescriptor(IPAddress source)
    {
      ...
      doc.SelectSingleNode(
            "//*[text() = 'urn:schemas-upnp-org:service:ContentDirectory:1']/../*[local-name() = 'controlURL']")
          .InnerText =
        $"{Prefix}control";
      ...
      doc.SelectSingleNode(
            "//*[text() = 'urn:schemas-upnp-org:service:ConnectionManager:1']/../*[local-name() = 'controlURL']")
          .InnerText
        =
        $"{Prefix}control";
      doc.SelectSingleNode(
            "//*[text() = 'urn:schemas-upnp-org:service:ConnectionManager:1']/../*[local-name() = 'eventSubURL']")
          .InnerText
        =
        $"{Prefix}events";
