...
namespace NMaier.SimpleDlna.Server.Views
{
  internal sealed class MusicView : BaseView
  {
    ...
    private static void LinkTriple(TripleKeyedVirtualFolder folder,
      ...
      string key2)
    {
      if (string.IsNullOrWhiteSpace(key1)) return;
      if (string.IsNullOrWhiteSpace(key2)) return;
      ...
      SimpleKeyedVirtualFolder genres)
    {
      ...
      foreach (var i in folder.ChildItems.ToList())
      {
        ...
        if (genre != null) genres.GetFolder(genre.StemNameBase()).AddResource(i);
      }
    }
    ...
    public override IMediaFolder Transform(IMediaFolder oldRoot)
    {
      ...
      foreach (var f in root.ChildFolders.ToList()) folders.AdoptFolder(f);
    ...
    private class AlbumInTitleAudioResource : AudioResourceDecorator
    {
      ...
      public override string Title
      {
        ...
        {
          var album = MetaAlbum;
          if (!string.IsNullOrWhiteSpace(album)) return $"{album} — {base.Title}";
          return base.Title;
        }
      }
    }
