...
namespace NMaier.SimpleDlna.Server.Views
{
  internal sealed class PlainView : BaseView
  {
    ...
    {
      foreach (var f in folder.ChildFolders.ToList()) EatAll(root, f);
      foreach (var c in folder.ChildItems.ToList()) root.AddResource(c);
    }

    public override IMediaFolder Transform(IMediaFolder oldRoot)
    {
      if (oldRoot == null) throw new ArgumentNullException(nameof(oldRoot));
