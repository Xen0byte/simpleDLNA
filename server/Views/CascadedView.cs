  ...
  internal abstract class CascadedView : BaseView, IConfigurable
  {
    ...
    public override IMediaFolder Transform(IMediaFolder oldRoot)
    {
      ...
      foreach (var f in sorted.ChildFolders.ToList())
      {
        if (f.ChildCount < 2)
        {
          foreach (var file in f.ChildItems) root.AddResource(file);
          continue;
        }
      ...
      foreach (var f in sorted.ChildItems.ToList()) root.AddResource(f);
      ...
      if (!cascade || root.ChildFolders.LongCount() <= 50) return root;
