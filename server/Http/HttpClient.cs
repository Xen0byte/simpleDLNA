  ...
  internal sealed class HttpClient : Logging, IRequest, IDisposable
  {
    ...
    private long GetContentLengthFromStream(Stream responseBody)
    {
      ...
      try
      {
            ...
            !long.TryParse(clf, out contentLength))
        {
          ...
          if (contentLength < 0) throw new InvalidDataException();
    ...
    private Stream ProcessRanges(IResponse rangedResponse, ref HttpCode status)
    {
      ...
      try
      {
        ...
        if (!m.Success) throw new InvalidDataException("Not parsed!");
        var totalLength = contentLength;
        long start;
        long end;
        if (!long.TryParse(m.Groups[1].Value, out start) || start < 0) throw new InvalidDataException("Not parsed");
        if (m.Groups.Count != 3 ||
        ...
        if (start > 0) responseBody.Seek(start, SeekOrigin.Current);
    ...
    {
      if (state == HttpStates.Closed) return;
      ...
      try
      {
        ...
        if (read < 0) throw new HttpException("Client did not send anything");
        ...
        if (!hasHeaders)
        {
            ...
            line = reader.ReadLine())
          {
            ...
            if (string.IsNullOrEmpty(line))
            {
              ...
              {
                if (bodyBytes > 1 << 20) throw new IOException("Body too long");
    ...
    private void SetupResponse()
    {
      ...
      try
      {
        if (!owner.AuthorizeClient(this)) throw new HttpStatusException(HttpCode.Denied);
        if (string.IsNullOrEmpty(Path)) throw new HttpStatusException(HttpCode.NotFound);
        var handler = owner.FindHandler(Path);
        if (handler == null) throw new HttpStatusException(HttpCode.NotFound);
        response = handler.HandleRequest(this);
        if (response == null) throw new ArgumentException("Handler did not return a response");
      }
