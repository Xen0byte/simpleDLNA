  ...
  public sealed class HttpServer : Logging, IDisposable
  {
    ...
    public Dictionary<string, string> MediaMounts
    {
      get
      {
        ...
        foreach (var m in servers) rv[m.Value.Prefix] = m.Value.FriendlyName;
        return rv;
      }
    }
    ...
    public void Dispose()
    {
      ...
      foreach (var s in servers.Values.ToList()) UnregisterMediaServer(s);
      ssdpServer.Dispose();
      timeouter.Dispose();
      listener.Stop();
      foreach (var c in clients.ToList()) c.Key.Dispose();
      clients.Clear();
    }
    ...
    {
      if (OnAuthorizeClient == null) return true;
      if (IPAddress.IsLoopback(client.RemoteEndpoint.Address)) return true;
    ...
    internal IPrefixHandler FindHandler(string prefix)
    {
      ...
      if (prefix == "/") return new IndexHandler(this);
    ...
    internal void RegisterHandler(IPrefixHandler handler)
    {
      ...
      if (FindHandler(prefix) != null) throw new ArgumentException("Invalid prefix; already taken");
      if (!prefixes.TryAdd(prefix, handler)) throw new ArgumentException("Invalid preifx; already taken");
    ...
    internal void UnregisterHandler(IPrefixHandler handler)
    {
      ...
      if (prefixes.TryRemove(handler.Prefix, out ignored)) DebugFormat("Unregistered Handler for {0}", handler.Prefix);
    }
    ...
    public void RegisterMediaServer(IMediaServer server)
    {
      ...
      if (servers.ContainsKey(guid)) throw new ArgumentException("Attempting to register more than once");
      ...
      foreach (var address in IP.ExternalIPAddresses)
      {
        ...
        var guidBytes = macBytes.Concat(new[] {addressByte}).Concat(serverBytes.Take(16 - 1 - macBytes.Length))
          .ToArray();
    ...
    public void UnregisterMediaServer(IMediaServer server)
    {
      if (server == null) throw new ArgumentNullException(nameof(server));
      MediaMount mount;
      if (!servers.TryGetValue(server.UUID, out mount)) return;
      ...
      if (devicesForServers.TryGetValue(server.UUID, out list))
      {
        lock (list)
        {
          foreach (var deviceGuid in list) ssdpServer.UnregisterNotification(deviceGuid);
        }
      ...
      MediaMount ignored;
      if (servers.TryRemove(server.UUID, out ignored)) InfoFormat("Unregistered Media Server {0}", server.UUID);
    }
  }
}
