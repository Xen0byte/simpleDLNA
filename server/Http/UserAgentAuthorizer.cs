...
namespace NMaier.SimpleDlna.Server
{
  public sealed class UserAgentAuthorizer : Logging, IHttpAuthorizationMethod
  {
    ...
    {
      if (userAgents == null) throw new ArgumentNullException(nameof(userAgents));
      foreach (var u in userAgents)
      {
        if (string.IsNullOrEmpty(u)) throw new FormatException("Invalid User-Agent supplied");
    ...
    public bool Authorize(IHeaders headers, IPEndPoint endPoint, string mac)
    {
      ...
      string ua;
      if (!headers.TryGetValue("User-Agent", out ua)) return false;
